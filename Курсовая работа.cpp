...
Stype* MenuStype(){
	...
	if (mode == 1){
		...
		if (number == 5){
			if (LengthStype(e)){
				REMEBMER = e->data;
				std::cout<<"Значение взятой очереди равно = " ;
		...
		if (number == 6){
			...
			if (equalsStype(e,read)) std::cout<<"Ошибка. Данная очередь уже есть в множестве.\n";
		...
		if (number == 7) equalsStype(e,MenuFtype())?std::cout<<"Данная очередь ЕСТЬ в множестве.\n":std::cout<<"Данной очереди НЕТ в множестве.\n";
